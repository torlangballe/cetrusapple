...
class ZGLShaderProgram {
    ...
    convenience init?(fvert:ZFileUrl, ffrag:ZFileUrl) {
        let (sv, _) = ZStrUtil.LoadFromFile(fvert)
        let (sf, _) = ZStrUtil.LoadFromFile(ffrag)
    ...
    func Add(_ source:String, vertex:Bool) -> Bool {
        ...
        source.enumerateLines { (part, quit) in
            nsource += "\n"
            if ZStrUtil.HasPrefix(part, prefix:"#include ", rest:&name) {
                let sinc = self.getIncludedText(ZStrUtil.Trim(name, chars:"\""))
                nsource += sinc
                let t = c + ZStrUtil.CountLines(sinc)
    ...
    func SetColorUniformValue(_ name:String, col:ZColor) {
        ...
        SetUniformArgF(4, name:name, a1:r, a2:g, a3:b, a4:a)
    }
