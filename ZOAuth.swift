...
class ZOAuth {
    ...
    func GetAuthorizationHeader(_ url:String, callbackUrl:String, get:Bool, urlArgs:[String:String]) -> String {
        ...
        let stimestamp = "\(ZTime.Now.timeIntervalSince1970)"
        ...
        sargs = ZStrUtil.UrlQuote(sargs)
        
        let ssignaturebase = shttpmethod + "&" + ZStrUtil.UrlQuote(url) + "&" + sargs
        let skey  = ZStrUtil.UrlQuote(sconsumerSecret) + "&" + ZStrUtil.UrlQuote(sauthTokenSecret)
